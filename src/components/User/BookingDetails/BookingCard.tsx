    ...
    return (
        <section className={`bg-[#F2F2F2] md:p-4 p-2 mx-2 my-3 shadow-md rounded-md font-Montserrat ${isExpanded && 'row-span-2'}`}>
            <div className='flex justify-between items-center text-[#252525e4] md:text-sm text-xs'>
                <h6 className='font-semibold'>Booking Id : <span>{bookedService._id}</span></h6>
            ...
            <div className='flex my-4'>
                <img src={(bookedService.serviceId as IService).image} className='w-[9rem] md:h-[9rem] h-[8rem] rounded-lg object-cover' alt="" />
                <div className='w-full  px-2.5 '>
                    <h3 className='font-semibold md:text-lg text-[0.9rem]'>{(bookedService.serviceId as IService).serviceName}</h3>
                    <div className='mt-2'>
                        <h6 className='font-semibold text-xs text-[#090808d4]'>Build/House name: <span className='font-medium'>{bookedService.buildingName}</span></h6>
                        <h6 className='font-semibold text-xs text-green-950 mt-3'>Start Time : <span>{bookedService.startTime}</span></h6>
                        <h6 className='font-semibold text-xs text-green-950'>End Time : <span>{bookedService.endTime}</span></h6>
                    </div>
                    <div className='hidden md:flex shadow-sm border justify-between rounded-lg py-1 px-2 mt-1'>
                        <div>
                        ...
                        <hr className=' my-2 border-[#b8b8b8]h-10 bg-slate-600' />
                        <div>
                            <h6 className='text-xs font-semibold'>Payment Status</h6>
                            <div className='flex items-center text-red-800'>
                                {paymentStatusIcon}
                                <h6 className='text-sm font-semibold mx-1'>{bookedService.paymentStatus}</h6>
                            </div>
                        </div>
                    </div>
                </div>
            ...
            <div className='md:hidden flex border justify-between bg-white rounded-lg py-1 px-2'>
                <div>
                    <h6 className='text-xs font-semibold'>Work Status</h6>
                    <div className='flex items-center text-red-800'>
                        {workStatusIcon}
            ...
            <div className='mt-1 px-2 py-1 w-full bg-white rounded-md shadow-md'>
